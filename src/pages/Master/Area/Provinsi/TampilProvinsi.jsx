...
const TampilProvinsi = () => {
  ...
  const tempPosts = provinsis.filter((val) => {
    if (searchTerm === "") {
      return val;
    } else if (
      val.id.toUpperCase().includes(searchTerm.toUpperCase()) ||
      val.namaProvinsi.toUpperCase().includes(searchTerm.toUpperCase())
    ) {
      return val;
    }
  });
  ...
  const _DATA = usePagination(provinsis, PER_PAGE);
  ...
  const deleteJaminan = async (id) => {
    ...
    try {
      ...
      if (error.response.data.message.includes("foreign key")) {
        alert(`${kodeProvinsi} tidak bisa dihapus karena sudah ada data!`);
      }
    }
    setLoading(false);
  };
  ...
  const downloadPdf = () => {
    ...
    doc.save("daftarProvinsi.pdf");
  };

  const { onDownload } = useDownloadExcel({
    currentTableRef: tableRef.current,
    filename: "Provinsi",
    sheet: "DaftarProvinsi"
  });
  ...
  return (
    <Container>
      <h3>Area</h3>
      <h5 style={{ fontWeight: 400 }}>Daftar Provinsi</h5>
      ...
      {previewPdf && (
        <div>
          ...
          <table class="table" id="table">
            ...
            <tbody>
              {provinsis.map((user, index) => (
                <tr key={user.id}>
                  <td>{user.id}</td>
                  <td>{user.namaProvinsi}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <Row>
              <Col sm={6}>
                ...
                >
                  ...
                  <Col sm="9">
                    <Form.Control value={namaProvinsi} disabled readOnly />
                  </Col>
                </Form.Group>
              </Col>
            </Row>
