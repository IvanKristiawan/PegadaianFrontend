...
import { ShowTableKabupaten } from "../../../../components/ShowTable";
...
const TampilKabupaten = () => {
  ...
  const tempPosts = kabupatens.filter((val) => {
    if (searchTerm === "") {
      return val;
    } else if (
      val.id.toUpperCase().includes(searchTerm.toUpperCase()) ||
      val.namaKabupaten.toUpperCase().includes(searchTerm.toUpperCase()) ||
  ...
  const _DATA = usePagination(kabupatens, PER_PAGE);
  ...
  const deleteKabupaten = async (id) => {
    setLoading(true);
    try {
      await axios.post(`${tempUrl}/deleteKabupaten/${id}`, {
        _id: user.id,
        token: user.token
      });
      getKabupatens();
      ...
      if (error.response.data.message.includes("foreign key")) {
        alert(`${kodeJenis} tidak bisa dihapus karena sudah ada data!`);
      }
    }
    setLoading(false);
  };
  ...
  const downloadPdf = () => {
    ...
    doc.save("daftarKabupaten.pdf");
  };

  const { onDownload } = useDownloadExcel({
    currentTableRef: tableRef.current,
    filename: "Kabupaten",
    sheet: "daftarKabupaten"
  });
  ...
  return (
    <Container>
      <h3>Area</h3>
      <h5 style={{ fontWeight: 400 }}>Daftar Kabupaten</h5>
      ...
      {previewPdf && (
        <div>
          ...
          <table class="table" id="table">
            ...
            <tbody>
              {kabupatens.map((user, index) => (
                <tr key={user.id}>
                  <td>{user.id}</td>
                  <td>{user.namaKabupaten}</td>
                  <td>
      ...
      <Box sx={buttonModifierContainer}>
        <Button
          ...
          size="small"
          onClick={() => {
            navigate(`/kabupaten/tambahKabupaten`);
          }}
        >
          Tambah
        </Button>
            ...
            >
              ...
              <DialogContent>
                <DialogContentText id="alert-dialog-slide-description">
                  {`Yakin ingin menghapus data ${kodeJenis}?`}
                </DialogContentText>
              </DialogContent>
              <DialogActions>
                <Button onClick={() => deleteKabupaten(id)}>Ok</Button>
            ...
            <Row>
              <Col sm={6}>
                ...
                >
                  ...
                  <Col sm="9">
                    <Form.Control value={kodeJenis} disabled readOnly />
                  </Col>
                </Form.Group>
              </Col>
            </Row>
            <Row>
              <Col sm={6}>
                ...
                >
                  ...
                  <Col sm="9">
                    <Form.Control value={namaKabupaten} disabled readOnly />
                  </Col>
                </Form.Group>
              </Col>
            </Row>
...
export default TampilKabupaten;
